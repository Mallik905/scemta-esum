...
public class HomeActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Button_muse_monitor =(Button) findViewById(R.id.button_to_muse);
        Button_muse_monitor.setVisibility(View.INVISIBLE);
        context = this;
        Button_muse_monitor.setOnClickListener(this);
        ...
        WeakReference<HomeActivity> weakActivity =
                new WeakReference<HomeActivity>(this);
    ...
    public void onClick(View v) {
        ...
        if (v.getId() == R.id.button_to_muse){
        ...
        if (v.getId() == R.id.refresh) {
            ...
            hint_text.setText(R.string.status1);
            ...
            if (availableMuses.size() < 1 || musesSpinner.getAdapter().getCount() < 1) {
                ...
                hint_text.setText(R.string.status5);
            } else {
                hint_text.setText(R.string.status6);
...
/*         else if (v.getId() == R.id.disconnect) {
  ...
  /*          final MuseVersion museVersion = muse.getMuseVersion();
              final TextView museVersionText = (TextView) findViewById(R.id.version);
                ...
                if(current == ConnectionState.CONNECTED){
                    try {
                        ...
                        connectionTimer.scheduleAtFixedRate(mTask, 20000, 20000);
                        alphaStale = true;
                        hint_text.setText(R.string.status2);
                    }
                    catch (Exception e){
                        e.printStackTrace();
                    }
        ...
        if (current == ConnectionState.DISCONNECTED) {
            ...
            alphaStale = false;
            connectionTimer.cancel();
            hint_text.setText(R.string.status7);
            findViewById(R.id.connect).setClickable(true);
        }
...
//                muse.disconnect();
//            }
//            System.out.println("Reconnect it again");
//            alphaStale = false;
            handler.post(myrun);
    ...
    private final Runnable myrun = new Runnable() {
        @Override
        public void run() {
            hint_text.setText(R.string.status3);
        }
    };
    /**
...
/*            case BATTERY:
    ...
    private void verifyAlphaBuffer(double[] alphaBuffer){
        try {
            String eeg1 = String.format("%6.2f", alphaBuffer[0]);
            String eeg2 = String.format("%6.2f", alphaBuffer[1]);
            String eeg3 = String.format("%6.2f", alphaBuffer[2]);
            String eeg4 = String.format("%6.2f", alphaBuffer[3]);
            System.out.println(eeg1);
            System.out.println(eeg4);
            if( (!eeg1.contains("NaN")) && (!eeg2.contains("NaN")) && (!eeg3.contains("NaN")) && (!eeg4.contains("NaN"))){
                connectionTimer.cancel();
                Toast.makeText(getApplicationContext(),"You are ready to use Muse now", Toast.LENGTH_SHORT).show();
                System.out.println("You are ready to use Muse now");
                alphaStale = false;
                Button_muse_monitor.setVisibility(View.VISIBLE);
                hint_text.setText(R.string.status4);
            }
...
/*    private void updateAccel() {
    ...
    class MuseL extends MuseListener {
        final WeakReference<HomeActivity> activityRef;

        MuseL(final WeakReference<HomeActivity> activityRef) {
    ...
    class ConnectionListener extends MuseConnectionListener {
        ...
        ConnectionListener(final WeakReference<HomeActivity> activityRef) {
            this.activityRef = activityRef;
        }
    ...
    class DataListener extends MuseDataListener {
        ...
        DataListener(final WeakReference<HomeActivity> activityRef) {
            this.activityRef = activityRef;
        }
